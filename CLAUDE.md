...
LLM provider is selected by `LLM_PROVIDER` (`gemini` default, `deepseek`, or `omniroute`):
...
- **omniroute** — needs `OMNIROUTE_API_KEY` and `OMNIROUTE_BASE_URL` (default `http://localhost:20128/v1`). Calls go through the OpenAI-compatible SDK, same shape as the deepseek path (LLooM embeddings also fall back to local `sentence-transformers`). OmniRoute is a self-hosted gateway that can also proxy consumer coding-tool subscriptions (Claude Code/Cursor/Codex) as raw API access — only point `CONFIG.models.omniroute_*_model` at model names backed by your own properly-licensed provider keys within that gateway, not the subscription-proxy routes, to stay within those providers' terms of service.
...
4. **LLooM Concept Scoring** (`pipeline_steps.py:run_lloom_iterative`) — Stanford LLooM induces high-level concepts from event narratives, then scores each event. Iterates up to 3× on uncovered rows (score == 0); a concept matching ≥50% of events (`generic_concept_max_coverage_fraction`) is treated as too generic and excluded. Outputs `score_results_combined.csv`.
...
- **`app/runner.py`** — `run_pipeline()` orchestrates all 6 steps with retry logic and cancellation support
...
- **`app/settings.py`** — Thin `SETTINGS` adapter exposing `config.py`'s `CONFIG` values under the attribute names used across `backend_service/` (step retry count 2, timeouts, LLooM knobs: max 5 concepts, 3 iterations)
...
`config.py` (repo root) is the single source of truth for tunable parameters via the `CONFIG` object: model names per provider (`CONFIG.models`), extraction batch/retry knobs, LLooM thresholds, score thresholds, and pipeline retry/timeout. `get_llm_provider()` reads `LLM_PROVIDER`. The backend's `app/settings.py` covers backend-service-specific runtime knobs; `config.py` covers the pipeline algorithm.
